/**
 * dsh-chinchilla-plugin-terminal — host half.
 *
...
import { appendFileSync, chmodSync, mkdirSync, readFileSync, statSync, unlinkSync, writeFileSync } from 'node:fs'
...
export const name = 'dsh-chinchilla-plugin-terminal'
...
export function apply(ctx) {
  ...
  function persistMeta() {
    try {
      ...
      try {
        chmodSync(DATA_DIR, 0o700) // POSIX-only; no-op where mode bits are ignored
      } catch {
        /* windows / permission quirks */
      }
      ...
      console.error('[dsh-chinchilla-plugin-terminal] persist meta failed:', err)
    }
  }

  /** One-time creation of the log dir with owner-only permissions. */
  function ensureLogDir() {
    mkdirSync(LOG_DIR, { recursive: true, mode: 0o700 })
    try {
      chmodSync(LOG_DIR, 0o700) // POSIX-only; no-op where mode bits are ignored
    } catch {
      /* windows / permission quirks */
    }
  }

  /** Append to a session log. Owner-only file permissions (the scrollback
   *  contains everything the user typed, including secrets) and a hard on-disk
   *  cap: logs outlive the in-memory ring buffer across restarts, so without a
   *  cap they would grow without bound. */
  ...
  function appendLog(id, chunk) {
    try {
      ...
      if (statSync(path).size > MAX_LOG_BYTES) {
        writeFileSync(path, readFileSync(path, 'utf8').slice(-SCROLLBACK_CHARS), { mode: 0o600 })
      }
    } catch (err) {
      console.error('[dsh-chinchilla-plugin-terminal] log write failed:', err)
    }
  }
      ...
      handler(req, socket, head) {
        ...
        wss.on('connection', (ws) => {
          ...
          ws.on('message', (data) => {
            ...
            if (text.length > 1_000_000) return // oversized input frames are dropped
  ...
  console.log('[dsh-chinchilla-plugin-terminal] host half active; routes under', PREFIX,
    '| toggleShortcut:', runtimeSettings.toggleShortcut,
