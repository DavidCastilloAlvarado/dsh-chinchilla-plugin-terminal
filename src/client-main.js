/**
 * dsh-chinchilla-plugin-terminal - client half (xterm.js, multi-tab, Codex-style bottom
...
const HEIGHT_KEY = "dsh-chinchilla-plugin-terminal.height";
const MIN_HEIGHT = 120;

/* xterm stylesheet served by the host plugin */
const XTERM_CSS_TAG = "dsh-chinchilla-plugin-terminal-xterm-css";
...
const STYLE_TAG = "dsh-chinchilla-plugin-terminal-styles";
...
function TermPane({ tab, active, onExit }) {
  ...
  useEffect(() => {
    ...
    term.loadAddon(new WebLinksAddon((_event, uri) => {
      /* terminal output is untrusted (the shell itself can print a
       * "link"): open plain web URLs only, never javascript:/file:/... */
...
function TerminalPanel(props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.error("[dsh-chinchilla-plugin-terminal] restore failed:", err);
      } finally {
        setBusy(false);
        setBootReady(true);
      }
    ...
    (async () => {
      try {
        ...
        if (typeof cfg.toggleShortcut === "string" && cfg.toggleShortcut.trim().length > 0) {
          ...
          else console.warn("[dsh-chinchilla-plugin-terminal] ignoring invalid toggleShortcut:", cfg.toggleShortcut);
        }
      } catch {
  ...
  const newTab = useCallback(async () => {
    ...
    try {
      ...
      console.error("[dsh-chinchilla-plugin-terminal] new tab failed:", err);
    } finally {
      setBusy(false);
    }
  ...
  const restartActive = useCallback(async () => {
    ...
    try {
      ...
      console.error("[dsh-chinchilla-plugin-terminal] restart failed:", err);
    } finally {
      setBusy(false);
    }
  }, [active, workspaceCwd]);
  ...
  const stateLabel = busy
    ? "Starting…"
    : active === null
      ? tabs.length === 0 ? "No session" : "Idle"
      : active.exited ? tabLabel(active) + " — exited, click ⟳ to restart" : tabLabel(active);
    ...
    open
      ? React.createElement(
          ...
          React.createElement(
              ...
              ...tabs.map((t) =>
                React.createElement(
                  "button",
                  {
                    ...
                    /* tooltip shows the tab's working directory so it's visible
                     * that each terminal follows the conversation's workspace */
                    title: (t.exited ? tabLabel(t) + " (exited)" : tabLabel(t)) + (t.cwd ? " — " + t.cwd : ""),
            ...
            active !== null
              ? React.createElement(
                  "button",
                  { className: "dshTermBarAction", title: active.exited ? "Restart process (keep tab)" : "Restart current session", "aria-label": "Restart current session", disabled: busy, onClick: restartActive },
                  Refresh14(),
                )
              : null,
            React.createElement(
              "button",
              {
                ...
                title: "Collapse panel (" + shortcutLabel + ")",
          ...
          React.createElement(
            ...
            tabs.length === 0
              ? React.createElement(
                  ...
                  React.createElement("span", null, "No terminal sessions"),
                  React.createElement(
                    "button",
      ...
      : React.createElement(
          "div",
          {
            ...
            title: "Terminal panel (toggle with " + shortcutLabel + ")",
            onClick: toggle,
            onKeyDown: (e) => {
          ...
          React.createElement("span", { className: "dshTermBarTitle" }, "Terminal" + (tabs.length > 1 ? " · " + tabs.length : "")),
